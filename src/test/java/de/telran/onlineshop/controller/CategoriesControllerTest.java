...
//        when(categoriesServiceMock.getCategoryById(testId)).thenReturn(new CategoryDto(testId,"Test"));
    ...
    void getCategoryByName() throws Exception {
        ...
        when(categoriesServiceMock.getCategoryByName(nameTest)).thenReturn(new CategoryDto(1L, nameTest));
this.mockMvc.perform(get("/categories/get?name=TestName", nameTest))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.categoryID").exists())
        .andExpect(jsonPath("$.categoryID").value(1))
